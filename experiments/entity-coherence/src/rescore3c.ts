// Rescore run-3c saved answers/projections against a revised key (default v5, the canonical
// key). No API calls. Writes results-run3c/rescored/{scores.json,scores.csv,trajectory.md} and
// prints every flipped probe score for hand-verification (run-1/2/3 audit discipline).
...
const keyFile = process.argv[2] ?? "probes-v5.json";
...
let md = `# Entity-model coherence — run 3c trajectory (C validation + four rules, rescored, ${keyFile})\n`;
