// Rescore run-2 saved answers/projections against a revised key (default v3).
// No API calls. Writes results-run2/rescored/{scores.json,scores.csv,trajectory.md}
// and prints every flipped probe score for hand-verification.
...
const keyFile = process.argv[2] ?? "probes-v3.json";
...
for (const policy of POLICIES) {
  for (let cycle = 1; cycle <= CYCLES; cycle++) {
    ...
    if (!existsSync(ansPath) || !existsSync(projPath)) {
    ...
    records.push({
      ...
      cycle,
      metrics,
      aux: { bytes: Buffer.byteLength(projection), noiseCount: noiseCount(projection, fixture.noiseItems), fabricatedDates: scan.count, fabricated: scan.fabricated },
      changed,
    });
  }
}
...
let md = `# Entity-model coherence — run 2 trajectory (N vs C, rescored, ${keyFile})\n`;
const fmt = (v: number | undefined) => (v === undefined ? "" : v.toFixed(3));
for (const metric of METRICS) {
  md += `\n## ${metric}\n\n| cycle | N | C |\n| --- | --- | --- |\n`;
  for (let cycle = 1; cycle <= CYCLES; cycle++) {
    const cells = POLICIES.map((p) => fmt(records.find((x) => x.policy === p && x.cycle === cycle)?.metrics[metric]));
    md += `| ${cycle} | ${cells.join(" | ")} |\n`;
  }
}
for (const aux of AUX) {
  ...
  for (let cycle = 1; cycle <= CYCLES; cycle++) {
    const cells = POLICIES.map((p) => {
      const v = records.find((x) => x.policy === p && x.cycle === cycle)?.aux[aux];
