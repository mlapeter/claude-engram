// Run 3b — accommodation REVALIDATION: C (conservative assimilation + ops-gated hygiene),
// C-only, 24 cycles, at the RETUNED ledger parameters per the run-3 verdict (README "Run 3" §3).
// Structurally identical to run3.ts (same fixture, prober, deterministic scorer, C op-engine,
// hygiene, per-cycle ledger trajectory); the ONLY differences are the three retunes wired here:
//   1. accommodation threshold 3.0 -> 0.8 (= base 1.0 × person accommodationInertia 0.8),
//   2. anchored surprise rubric in the assimilation prompt (C_SYSTEM_RUN3B), and
//   3. probe max_tokens 600 -> 2000 (PROBE_MAX_TOKENS_RUN3B) to fix the run-3 probe abort.
// Scores against the pre-registered v4 key at write time; canonical numbers are the v5 rescore
// (src/rescore3b.ts). Writes to results-run3b/ (run 3's results-run3/ is left untouched).
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { PROBE_CONCURRENCY, PROBE_SYSTEM, probeUserPrompt } from "./config.ts";
import {
  ALL_POLICIES3B,
  ACCOMMODATE_THRESHOLD_RUN3B,
  C_SYSTEM_RUN3B,
  cUserPrompt,
  DEFAULT_POLICIES_RUN3B,
  HYGIENE_CYCLES,
  MAINTAIN_MAX_TOKENS_N,
  N_SYSTEM,
  nUserPrompt,
  OPS_MAX_TOKENS_C,
  PROBE_MAX_TOKENS_RUN3B,
  TOTAL_CYCLES_RUN3B,
  type Policy3B,
} from "./run3b-config.ts";
import {
  applicableKey,
  EXPERIMENT_DIR,
  loadProbes,
...
const RESULTS_DIR = join(EXPERIMENT_DIR, "results-run3b");
const pad = (n: number) => String(n).padStart(2, "0");

type Args = { policies: Policy3B[]; cycles: number; dryRun: boolean };

function parseArgs(argv: string[]): Args {
  let policies: Policy3B[] = [...DEFAULT_POLICIES_RUN3B]; // C-only revalidation by default.
  let cycles = TOTAL_CYCLES_RUN3B;
  let dryRun = false;
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--dry-run") dryRun = true;
      ...
      policies = v.split(",").map((p) => p.trim()) as Policy3B[];
      for (const p of policies) if (!ALL_POLICIES3B.includes(p)) throw new Error(`Unknown policy: ${p}`);
    } else if (a === "--cycles") {
      const v = argv[++i];
      if (!v) throw new Error("--cycles requires a value");
      cycles = Number(v);
      if (!Number.isInteger(cycles) || cycles < 1 || cycles > TOTAL_CYCLES_RUN3B) {
        throw new Error(`--cycles must be an integer in 1..${TOTAL_CYCLES_RUN3B}`);
      }
...
type AnswersFile = { policy: Policy3B; cycle: number; date: string; probes: ProbeAnswer[] };

type ScoreRecord = {
  policy: Policy3B; cycle: number; metrics: Record<string, number>;
...
const p = {
  projection: (policy: Policy3B, cycle: number) => join(RESULTS_DIR, "projections", policy, `cycle-${pad(cycle)}.md`),
  answers: (policy: Policy3B, cycle: number) => join(RESULTS_DIR, "answers", policy, `cycle-${pad(cycle)}.json`),
...
async function proposeOps(state: ModelState, cycle: number, date: string, transcript: string): Promise<Op[]> {
  ...
  const first = await callModel({ system: C_SYSTEM_RUN3B, user, maxTokens: OPS_MAX_TOKENS_C, label });
  try {
    ...
    const second = await callModel({ system: C_SYSTEM_RUN3B, user: retryUser, maxTokens: OPS_MAX_TOKENS_C, label: `${label} retry` });
...
async function probeAndScore(
  fixture: ProbesFile, policy: Policy3B, cycle: number, date: string, projection: string,
): Promise<AnswersFile> {
  ...
  const answered = await mapWithConcurrency(applicable, PROBE_CONCURRENCY, async ({ probe, keyCycles }) => {
    const answer = await callModel({
      ...
      maxTokens: PROBE_MAX_TOKENS_RUN3B, label: `probe ${probe.id} ${policy} cycle ${cycle}`,
    });
    const key = applicableKey(probe, cycle)!;
...
function computeScoreRecord(
  fixture: ProbesFile, sessionDates: string[], policy: Policy3B, cycle: number, projection: string, answers: AnswersFile,
): ScoreRecord {
...
function writeSummaries(records: ScoreRecord[], policies: Policy3B[], cycles: number) {
  ...
  const out: string[] = ["# Entity-model coherence — run 3b trajectory (C revalidation, v4 key, threshold 0.8)", ""];
...
function writeLedgerTrajectory(ledgerCycles: LedgerCycle[], threshold: number) {
  ...
  const out: string[] = [
    `# Ledger trajectory — run 3b (C revalidation), accommodation threshold = ${threshold.toFixed(1)} (= 1.0 × person inertia 0.8)`, "",
...
function dryRun(fixture: ProbesFile, cycles: number) {
  for (const probe of fixture.probes) {
    for (const [from, to] of probe.keys.map((k) => k.cycles)) {
      if (from < 1 || to > TOTAL_CYCLES_RUN3B || from > to) throw new Error(`Probe ${probe.id}: invalid cycle range [${from}, ${to}]`);
    }
  }
  ...
  // Exercise the C op path (threshold 0.8) end-to-end.
  ...
  ], 1, sessions[0].date, ACCOMMODATE_THRESHOLD_RUN3B);
  if (r1.outcomes.some((o) => o.outcome === "rejected")) throw new Error("dry-run C op self-check failed");
  // bel-1 must DEFER (strong×0.8×medium = 1.0×0.8×0.75 = 0.6 < 0.8), not accommodate on one op.
  if (!r1.outcomes.some((o) => o.op.op === "supersede" && o.outcome === "deferred")) {
    throw new Error("dry-run: expected belief supersede to DEFER under threshold 0.8");
  }
  ...
  console.log(`Dry run OK (C threshold ${ACCOMMODATE_THRESHOLD_RUN3B}; auto-pin, sacred-verbatim, no-re-derivation guards all refused correctly).`);
}
  ...
  for (const policy of args.policies) {
    ...
    for (let cycle = 1; cycle <= args.cycles; cycle++) {
      ...
      if (resumable) {
        ...
        // C: assimilate (threshold 0.8), then ops-gated hygiene on hygiene cycles.
        const ops = await proposeOps(state, cycle, session.date, session.transcript);
        const applied: { outcomes: OpOutcome[] } = applyOps(state, ops, cycle, session.date, ACCOMMODATE_THRESHOLD_RUN3B);
        ...
        writeFileSync(p.ops(cycle), JSON.stringify({ cycle, date: session.date, threshold: ACCOMMODATE_THRESHOLD_RUN3B, outcomes: applied.outcomes }, null, 2) + "\n");
        ...
        const lc: LedgerCycle = { cycle, date: session.date, threshold: ACCOMMODATE_THRESHOLD_RUN3B, snapshot: ledgerSnapshot(state), accommodatedThisCycle, hygieneRan };
      ...
      if (policy === "C") writeLedgerTrajectory(ledgerCycles, ACCOMMODATE_THRESHOLD_RUN3B);
    }
  }
