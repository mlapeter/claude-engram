// Audit helper for run 3: dump every sub-1.0 probe answer (with the key that scored it) and
// every fabricated-date hit, for hand-reading. No API calls. (run-1/2 discipline: never trust a
// keyword score until its failures are read.)
...
const fixture = loadProbes(process.argv[2] ?? "probes-v4.json");
const CYCLES = 24;
